...
### 🖼️ 2. 뷰어 (Viewer) 모드
- **다양한 읽기 모드**:
  - `우측에서 좌측으로 읽기(RTL, 기본)` / `좌측에서 우측으로 읽기(LTR)` 전환 (`D` 키)
  - `1장 보기` / `2장 보기` 양면 펼침 전환 (`1`, `2` 키)
  - `스프레드 순서 뒤집기` (`C` 키)
  - `가로로 꽉 차게 보기` (`W` 키) 및 페이지 넘김 시 스마트 자동 스크롤
- **스마트 줌 & 자연스러운 패닝**:
  - 더블 클릭 / 마우스 휠 / 트랙패드 핀치 제스처 / `Z` 키로 확대/축소
...
- **고품질 화질 향상 필터**:
  - **Lanczos3 고화질 리샘플링 & 샤프닝(선명도)** 조절 (`S` 키)
...
| 단축키 | 기능 설명 |
| :--- | :--- |
| `Space` / `Enter` | 다음 페이지 (우→좌 읽기 기준 왼쪽 / 좌→우 기준 오른쪽) |
| `←` / `→` | 이전 / 다음 페이지 이동 (확대 시에는 패닝 또는 페이지 넘김) |
| `↑` / `↓` | 상/하 스크롤 및 확대 상태 패닝 |
| `1` / `2` | 1장 보기 / 2장 보기 전환 |
| `D` | 읽기 방향 전환 (우측에서 좌측 ⇄ 좌측에서 우측) |
| `C` | 2장 보기 펼침면 순서 뒤집기 |
| `W` | 가로로 꽉 차게 보기 토글 |
| `Z` | 스마트 줌 토글 (기본 2.0x) |
| `S` | 선명도(샤프닝) 조절 (끄기 → 약하게 → 보통 → 강하게) |
| `A` | 흑백 만화 자동 대비(Auto Contrast) 보정 토글 |
| `[` / `]` | 이전 권 / 다음 권 이동 |
| `F` | 전체화면 모드 토글 |
| `ESC` / `Backspace` | 책장으로 돌아가기 |
